...
void loop() {
  ...
  while (gpsSerial.available() > 0) {
    Serial.println("Capturar dados do GPS");
    gps.encode(gpsSerial.read());
  }

  if (gps.location.isUpdated()) {
    Serial.println("if (gps.location.isUpdated())");
...
*/
    ...
    if (Firebase.RTDB.setJSON(&fbdo, "/vehicles/veiculo5", &json)) {
      Serial.print("lat ");
